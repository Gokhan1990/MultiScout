...
async def scrape_teknosa_deals(
    output_file: str,
    ...
    deals: list[dict] = []
    url = f"https://www.teknosa.com/arama/?s={quote_plus(category)}"
    print(f"[Teknosa] Başlıyor: {url}", flush=True)
    ...
    try:
        async with async_playwright() as p:
            ...
            print(f"[Teknosa] {category}: {raw_count} ham, {kept} kept", flush=True)
            await browser.close()

    except Exception as e:
        print(f"[Teknosa] HATA ({category}): {e}", flush=True)

    async with get_file_lock(output_file):
        existing = load_deals(output_file)
        merged = merge_deals_by_link(existing, deals)
        print(f"[Teknosa] Toplam {len(merged)} ürün kaydediliyor (yeni: {len(deals)})...", flush=True)
